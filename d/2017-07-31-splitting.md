---
title: 'Is it ever reasonable to split one's donations?'
layout: post
---
...
The simplest analysis is to look at a histogram of the "allocation percentage" variable. The result looks like this:
...
I compute a histogram of allocation percentages weighted by donation size. In other words, I ask: "if I pick a random dollar flowing through EA funds, what is its probability of being part of an EA funds donation which itself represents X% of an EA funds allocation?", and then plot this for 20 buckets of Xs.

![lessnaive](../images/lessnaive.png)

The data:
allocation percenttage bucket	|	probability
--- | ---
5%	|	0.009
10%	|	0.011
15%	|	0.025
...
This allows me to create a histogram of the fraction of a user total represented by each fund sum, weighted by the fund sum.
...
What if you also hold some credence in merely ordinal theories? In that case you should treat the situation as a voting problem. Each theory can "vote" by ranking your possible actions, and gets a number of votes that is proportional to your credence in that theory. 
...
It seems that the view that one's preferred focus area is orders of magnitude better than others, should generally imply the view that the whole EA community should donate only to that focus area. Of course, some areas may have much faster diminishing returns than others[^exemp].
...
I find this type of argument from bias unconvincing[^also]. I suspect that the biases involved are insensitive to the scope of the donations, that is, it's sufficient to donate a nomial amount to other causes in order to reduce or eliminate the bias. Then such considerations would offer no reason for significant splitting. It's also questionable whether such self-deception is even [likely to work](http://lesswrong.com/lw/je/doublethink_choosing_to_be_biased/). Claire Zabel's post ["How we can make it easier to change your mind about cause areas"](http://effective-altruism.com/ea/m3/how_we_can_make_it_easier_to_change_your_mind/) also offers five techniques for reducing this bias. Applying these techniques seems like a far less costly approach than sacrificing significant expected impact to splitting.
...
* There could be cheaper ways of achieving the same benefits. For example, saying "This year I donated 100% to X, but in the past I've donated to Z and Y" or "Many of my friends in the community donate to Z and Y" could send the intended signals without requiring any actual splitting.

# Recommendation
Many of the explicit reasons given for splitting turn out to be unconvincing. Cooperation with donors appears to me to be the most convincing reason for splitting. 

The fact that many ostensive reasons for splitting don't bear scrutiny suggests that we may often be using splitting to satisfy our urge to [purchase "fuzzies"](http://lesswrong.com/lw/6z/purchase_fuzzies_and_utilons_separately/). And I say this without negative judgement, I agree with Claire Zabel that we should "de-stigmatize talking about emotional attachment to causes". It may not be psychologically realistic to plan to stop splitting altogether. Instead, one could give as much as possible to the recipient with the highest expected value, while satisfying our desire to split with the small remaining part. Personally, I donate 90% to the Far Future EA fund and 10% to the Animal Welfare fund for this reason. 
